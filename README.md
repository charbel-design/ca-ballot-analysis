...
Three ways to use this, in order of effort:

1. **[Paste-in prompt](PROMPT-VERSION.md)** (easiest, works with any Claude account including free tier). Copy one prompt, paste into any Claude conversation, point it at your ballot. No installation.

2. **[Full usage guide](USAGE.md)** (three paths explained). Read-and-apply manually, paste-in prompt, or full skill installation.

3. **Fork this repo and contribute a framework.** See the Contributing section below.
...
The framework is always explicit. A progressive framework and a libertarian framework will produce different picks from the same ballot. That's the feature.
...
4. **Feasibility multiplier.** Policy quality times feasibility, not plus. A great platform that cannot pass is worth less than a good platform that can.
5. **Confidence calibration.** Every pick is High, Medium, or Low confidence. Low-confidence picks explicitly invite pushback.
...
- **fairness-not-equity.md** — Common-sense fairness, rule of law, individual over group, five-year test. Written, production-ready. Includes the authored-pork sub-test contributed by Akram AbouKhalil.
...
.
├── README.md                          (this file)
├── USAGE.md                           (three usage paths explained)
├── PROMPT-VERSION.md                  (paste-in prompt for any Claude user)
├── SKILL.md                           (skill trigger and workflow for installation)
├── LICENSE                            (MIT)
├── frameworks/
│   ├── fairness-not-equity.md
│   ├── progressive.md
│   ├── libertarian.md
│   ├── yimby-housing.md
│   └── custom-template.md
├── research/
│   ├── sources.md                     (four-tier source hierarchy)
│   ├── character-gate.md              (gate evaluation guide)
│   └── measure-analysis.md            (ballot measure patterns)
├── scoring/
│   ├── weighted-criteria.md           (scoring workflow)
│   └── confidence-levels.md           (High/Medium/Low calibration)
├── deliverables/
│   ├── full-report.md                 (Word doc spec)
│   ├── one-sheet.md                   (PDF spec)
│   └── conversational.md              (default chat output spec)
└── examples/
    └── 2026-primary-ca10.md           (worked example)
```
...
4. Submit a pull request with a brief author note

Every framework stays attributed to its author. The authored-pork sub-test in the fairness-not-equity framework credits Akram AbouKhalil directly. Yours would be credited the same way.

For framework improvements, factual corrections, or methodology refinements, open an issue. Short precise proposals beat long abstract ones.
...
- **Recent candidates** may have information gaps
- **District assignments** can be wrong if the skill uses outdated maps (always verify against your sample ballot under the 2025 Prop 50 redistricting)
...
The alternative to an explicit-framework ballot analyzer is not "voters thinking for themselves, unaided." It is voters outsourcing thinking to party endorsements, union recommendations, editorial boards, or AI tools that hide their frameworks.

This skill puts the framework in the open so users can interrogate it, modify it, or reject it. Framework-based analysis is not a substitute for civic engagement. It is a tool for voters who care enough to want their reasoning explicit.
